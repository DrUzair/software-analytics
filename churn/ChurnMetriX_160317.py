...
# 733687 files related to 170332 commits
...
def main():
    ...
    global churn_file
    churn_file = 'Churn_Data_Test' + str(start) + '-' + str(end) + '.tdl'
    ids = []
    for i in range(start, end):
        ids.append(i)
    process_commit_ids(start)
    ...
    results = pool.map(process_commit_ids, ids)
    #
...
def read_raw_file_from_mercurial(commit_id, commit_record_file_name, file_type, the_url_raw_file):
    ...
    file_path = raw_file_dir + '\\'+commit_record_file_name.replace('/', '-') + '.raw'
    if (not os.path.isfile(file_path)):
        done = download_file(the_url_raw_file, file_path)
        if not done: # encountered error in downloading
            return(0,0)
    return get_line_of_code_comments(file_path, file_type)

def download_file(the_url, file_path):     # if file is not already downloaded, then download and store at the file_path
    if (not os.path.isfile(file_path)):
        f_raw_file = open(file_path, 'w')  # overwriting existing file
        try:
            #for line in url_open(the_url):
            for line in urllib2.urlopen(the_url):
                f_raw_file.write(line)
        except urllib2.HTTPError:
            print 'Invalid Raw-File URL or something --> ' + the_url
            f_raw_file.close()
            return False
        except:
            print 'UNResolved ERROR downloading --> ' + the_url
            f_raw_file.close()
            return False
        f_raw_file.close()
    return True

def get_line_of_code_comments(file_path, file_type):
    f_raw_file = open(file_path, 'r')  # overwriting existing file
    lines_of_code = 0
    lines_of_comment = 0
    prev_line_was_comment = False
    for line in f_raw_file.readlines():
        if (len(line.strip()) > 0):
...
def read_diff_from_mercurial(commit_id):
    the_url_diff_file = url_prefix_rawrev + commit_id
    file_path = rawdata_dir + "raw-diffs\\" + commit_id + '.rawdiff'
    if (not os.path.isfile(file_path)):
        done = download_file(the_url_diff_file, file_path)
        if not done:
            print "Error downloading --> " + the_url_diff_file
    return parse_diff_rawrev_file(file_path, commit_id)

def parse_diff_rawrev_file(file_path, commit_id):
    ...
    hunk_started = False #@@
    diff_started = False #diff --git /a has been detected
    file_type_ok = True
    ...
    prev_line_was_comment = False
    #is_first_diff = True
    f_rawdiff_file = open(file_path, 'r')

    for line in f_rawdiff_file.readlines():
        line = line.strip()
        if (line.find("# User ") != -1):
        ...
        if(line.startswith('diff --git a/')):
            ...
            continue # to the next line
        if(file_type_ok and diff_started):
...
def process_commit_ids(commit_id_number):
    # url_prefix is taken from mercurial
    commit_id_len = 12
    f_commit_ids = open(churn_dir + 'commit_ids_mar2017.txt', 'r')
    f_commit_ids_lines = f_commit_ids.readlines()
    for i in range(commit_id_number, commit_id_number+1): #
        #print "commit # " + str(i)
        commit_id = f_commit_ids_lines[i].split('\t')[0].strip()
        bug_id = f_commit_ids_lines[i].split('\t')[1].strip()

        churn_obj_list = read_diff_from_mercurial(commit_id)

        method_names = ""
        for churn_obj in churn_obj_list:
            churn_obj.bug_id = bug_id
            ...
            f_churn_data = None
            if (not os.path.isfile(churn_dir + churn_file)):
                f_churn_data = open(churn_dir + churn_file, 'a')
                ...
                f_churn_data = f_churn_data = open(churn_dir + churn_file, 'a')
            f_churn_data.write(str(i) + '\t' +
                           ...
                           method_names + '\n')
            f_churn_data.close()
    #if (commit_id_number % 100 == 0 ):
    print (str(commit_id_number) + ' commits ' + 'completed')
...
''' LIMITATIONS
Some diffs may not follow the pattern for example https://hg.mozilla.org/mozilla-central/raw-rev/a9d2ba227725
does not have "diff --git /a" entry for the first file

Hundreds of files are removed from repo. e.g.
https://hg.mozilla.org/mozilla-central/raw-file/ab946a12992a/mobile/android/base/db/AndroidBrowserDB.java
'''